...
internal class TestFragmentManager {
  ...
  fun assertFragmentRemoved(fragment: Fragment) {
    verify(fragmentTransaction).remove(fragment)
    verify(fragmentTransaction, atLeastOnce()).commit()
  }
}

internal inline fun <reified T : Fragment> TestFragmentManager.assertFragmentAdded(): Pair<Fragment, String> {
